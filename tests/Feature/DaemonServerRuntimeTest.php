...
test('daemon can report an install failure for a server', function () {
    ...
    expect($server->fresh()->last_error)->toBe(
        'Installation failed with exit code 2.',
    );
});

test(
    'daemon can update server runtime state and clear a previous error',
    function () {
        $server = daemonRuntimeServer();
        $server
            ->forceFill([
...
test(
    'daemon cannot report runtime updates for a server on another node',
    function () {
        $server = daemonRuntimeServer();
