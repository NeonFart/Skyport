...
test(
    'octane sensitive services do not capture request scoped dependencies in constructors',
    function () {
        collect([CreateNewUser::class, PasskeyService::class])->each(function (
            string $class,
        ): void {
            ...
            $parameterTypes = collect($constructor->getParameters())
                ->map(
                    fn (ReflectionParameter $parameter): ?string => $parameter
                        ->getType()
                        ?->getName(),
                )
                ->filter()
